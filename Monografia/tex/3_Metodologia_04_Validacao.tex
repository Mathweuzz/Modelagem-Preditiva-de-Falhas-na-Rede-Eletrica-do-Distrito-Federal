...
Persistência & --- & 365 pares & $\widehat y_{t+1}=y_t$ \\
\bottomrule
\end{tabularx}
...
Nenhuma métrica isolada determina superioridade absoluta. O MAE é diretamente interpretável; o RMSE revela sensibilidade a picos; $R^2$ mede variância explicada em relação à média; e o MAPE pondera relativamente dias de menor volume. Diferenças pequenas devem ser discutidas considerando essas propriedades e a ausência de um teste inferencial de diferença entre modelos.
...
As métricas são pontuais e não foram acompanhadas por intervalos de confiança ou testes pareados de diferença. Assim, pequenas diferenças numéricas não devem ser tratadas como prova estatística de superioridade. A comparação conjunta de MAE, RMSE, $R^2$, MAPE e persistência oferece evidência descritiva, não uma ordenação universal.
