...
Para manter rastreabilidade entre os objetivos do trabalho e a execução, cada questão foi associada a uma operação e a um conjunto de evidências. A Tabela~\ref{tab:objetivos_evidencias} não acrescenta novas hipóteses; ela mostra onde cada conclusão encontra suporte empírico.

\begin{table}[H]
\centering
\caption{Relação entre objetivos, operações e evidências}
\label{tab:objetivos_evidencias}
\small
