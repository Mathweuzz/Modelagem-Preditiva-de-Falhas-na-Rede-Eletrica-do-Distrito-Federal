...
Implementação do XGBoost com hiperparâmetros pré-selecionados após uma fase exploratória de \textit{Grid Search} sobre \texttt{TimeSeriesSplit} (5 \textit{folds}): $n\_estimators = 500$, $learning\_rate = 0{,}05$, $max\_depth = 6$, $subsample = 0{,}8$ e $colsample\_bytree = 0{,}8$, com \texttt{random\_state} fixado em 42 para reprodutibilidade. O módulo também define a função genérica \texttt{evaluate\_and\_plot}, reutilizada pelos modelos Bi-LSTM e Bi-GRU.
...
Script que implementa a avaliação dos três modelos nos horizontes de 1, 3, 7 e 14 dias (\texttt{Fonte/src/models/previsao\_multihorizonte.py}). Para o XGBoost, utiliza previsão direta com as \textit{features} pré-computadas de cada dia. Para Bi-LSTM e Bi-GRU, aplica a estratégia recursiva auto-regressiva: a previsão de $t+k$ é obtida alimentando a estimativa de $t+k-1$ de volta para a janela de entrada, acumulando os erros progressivamente. Os modelos são treinados uma única vez sobre os 3.044 dias anteriores ao período de avaliação de 14 dias. A listagem completa integra o pacote de código-fonte entregue.
