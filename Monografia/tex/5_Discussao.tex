...
Em contrapartida, as redes recorrentes demonstraram uma propriedade relevante na distribuição residual (Seção~\ref{sec:residuais}): suas distribuições KDE apresentam maior densidade próxima de zero. Essa forma, isoladamente, não implica menor MAE nos dias de menor volume; a análise segmentada mostra que o XGBoost obteve o menor MAE nas faixas de baixo e alto volume, enquanto a Bi-LSTM foi ligeiramente melhor na faixa intermediária. Com dados adicionais e em cenários de maior complexidade temporal (resolução horária, por exemplo), permanece como hipótese de trabalho avaliar se as redes recorrentes poderiam superar o XGBoost.
...
No recorte comum, em $h = 1$, XGBoost e Bi-LSTM apresentam resultados próximos (MAE de 62,5 e 64,2, respectivamente), com o XGBoost ligeiramente melhor em MAE e a Bi-LSTM ligeiramente melhor em $R^2$; ambos também registram MAE abaixo da persistência (69,8). Para $h = 3$ dias, a Bi-LSTM registra o menor MAE entre os modelos treinados (74,0, contra 78,8 do XGBoost e 77,7 da Bi-GRU), todos abaixo da persistência (86,2). A Bi-GRU registra os menores MAEs entre os modelos treinados nos horizontes mais longos: 73,6 em $h=7$ e 64,3 em $h=14$, também abaixo da persistência (84,6 e 88,0). Em particular, o XGBoost registra $R^2 = -0{,}027$ em $h=7$, indicando desempenho inferior ao \textit{baseline} da média nesse horizonte específico.

\begin{itemize}
    \item O menor MAE da \textbf{Bi-LSTM} em $h=3$ é compatível com o aproveitamento da janela recente, mas o experimento não isola esse mecanismo arquitetural de outras diferenças de treinamento e representação.
    \item Os menores MAEs da \textbf{Bi-GRU} em $h=7$ e $h=14$ motivam a hipótese de que sua parametrização mais compacta tenha favorecido a generalização nesse recorte; uma ablação específica seria necessária para testar essa explicação.
...
No conjunto avaliado, XGBoost e Bi-LSTM ficaram próximos em $h=1$, enquanto uma rede recorrente registrou o menor MAE entre os modelos treinados em cada horizonte $h \geq 3$. Uma eventual combinação operacional --- XGBoost ou Bi-LSTM no curtíssimo prazo e Bi-GRU em $h=7$ e $h=14$ --- deve ser validada prospectivamente antes da implantação.
