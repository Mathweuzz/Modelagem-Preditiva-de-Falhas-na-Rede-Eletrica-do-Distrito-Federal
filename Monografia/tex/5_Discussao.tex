...
\section{Papel da Interface na Reprodutibilidade}\label{sec:discussao_interface}

A interface reduz a barreira operacional para consultar a base, comparar modelos e iniciar tarefas do projeto. Sua principal contribuição não é melhorar as métricas, mas tornar mais visíveis as escolhas do experimento e os artefatos produzidos. A separação entre resultados oficiais e execuções interativas é especialmente relevante: uma nova configuração pode ser testada sem substituir as previsões que fundamentam as tabelas e conclusões desta monografia.

Essa facilidade de uso não transforma o protótipo em sistema operacional da distribuidora. A aplicação depende da base processada localmente, não incorpora atualização automática de dados nem previsões meteorológicas emitidas no momento da decisão e não foi submetida a validação prospectiva. Também não possui autenticação, monitoramento contínuo ou integração com sistemas da concessionária. A presença de botões para treinamento e comparação não elimina a necessidade de examinar datas, variáveis, hipóteses e incerteza. Portanto, seu uso adequado é acadêmico, demonstrativo e de apoio à reprodução dos experimentos.

A opção ARIMAX disponível na interface tem caráter experimental. Como ela emprega covariáveis observadas na origem e não participou da avaliação oficial comum, não há fundamento neste estudo para compará-la numericamente aos três modelos centrais ou recomendar sua adoção. Sua permanência na aplicação oferece uma extensão exploratória claramente identificada, sem ampliar retroativamente o escopo empírico das conclusões.
...
Os resultados deste trabalho indicam possíveis aplicações no apoio ao planejamento operacional da concessionária:
...
À luz das evidências obtidas, a comparação entre os modelos deve ser interpretada com cautela, considerando a proximidade de desempenho, a estrutura temporal remanescente nos resíduos e as limitações dos dados disponíveis. A etapa final retoma os objetivos da pesquisa, sintetiza suas principais contribuições e delimita as possibilidades de continuidade. O próximo capítulo apresenta as conclusões deste trabalho.
