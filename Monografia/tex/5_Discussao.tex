...
Sob o protocolo unificado de previsão direta $h=1$ --- mesmas variáveis por instante, corte temporal e datas-alvo, mas representação tabular para o XGBoost e sequencial para as redes --- a \textbf{Bi-GRU} apresentou o menor MAE (60,36) e MAPE (18,89\%), enquanto a Bi-LSTM obteve o menor RMSE (98,70) e o maior $R^2$ (0,421). O XGBoost teve MAE de 63,81. Os três modelos treinados superaram a persistência nas quatro métricas.

O resultado também não sustenta superioridade arquitetural universal: as duas redes lideraram métricas diferentes, e o XGBoost permaneceu competitivo sob uma representação tabular. Seu desempenho pode estar associado a três fatores complementares:

\begin{enumerate}
    \item \textbf{Engenharia de atributos temporais:} As 40 variáveis de entrada --- sete meteorológicas base e 33 derivadas, incluindo defasagens em 1, 2, 3 e 7 dias, médias móveis exponenciais e codificações cíclicas --- fornecem informação temporal explícita. A série-alvo histórica forma um canal adicional disponível na origem. Uma ablação seria necessária para quantificar a contribuição de cada grupo.
...
\midrule
Acurácia Geral ($h=1$, 365 dias) & $R^2 = 0{,}385$, MAE $= 63{,}81$ & Bi-GRU: MAE $= 60{,}36$; Bi-LSTM: $R^2 = 0{,}421$ \\
Previsão $h>1$ (direto) & Competitivo, sem menor MAE agregado & Bi-LSTM lidera $h=3$; Bi-GRU lidera $h=7$ e $h=14$ \\
...
Representação de entrada & Variáveis em uma linha na origem & Mesmas variáveis + janela de 14 dias \\
\bottomrule
\end{tabular}
\normalsize
\end{center}
...
As distribuições residuais (Seção~\ref{sec:residuais}) permanecem centradas próximas de zero, mas apresentam caudas associadas aos maiores valores reais. Na faixa Normal, a Bi-GRU obteve o menor MAE; a Bi-LSTM liderou na Moderada, e o XGBoost na Severa. As 50 observações severas permitem o diagnóstico do aumento de erro, embora não substituam validação prospectiva. Com dados adicionais, permanece como hipótese avaliar se redes recorrentes poderiam apresentar vantagem em outros regimes.
...
No recorte comum, a Bi-GRU apresenta o menor MAE em $h=1$ (61,67), $h=7$ (71,24) e $h=14$ (68,19). Em $h=3$, a Bi-LSTM apresenta o menor MAE (65,56). A persistência registra 69,83, 86,22, 84,56 e 88,00, respectivamente; todos os modelos treinados a superam nos quatro horizontes.

\begin{itemize}
    \item A \textbf{Bi-GRU} registra o menor MAE em três dos quatro horizontes, mas não domina todos os meses do período de teste.
    \item A vantagem agregada das redes varia por horizonte e não sustenta superioridade arquitetural universal sem repetição com outras sementes e períodos.
...
No conjunto avaliado, a Bi-GRU apresentou o menor MAE em três horizontes, e a Bi-LSTM em um. Qualquer política operacional de seleção ou combinação deve ser validada prospectivamente antes da implantação.

A decomposição mensal qualifica essa recomendação: o XGBoost obtém o menor MAE em 6 dos 12 meses em $h=1$ e 3 em $h=3$; nos horizontes 7 e 14, cada arquitetura vence quatro meses. Como a contagem de vitórias não pondera a magnitude do erro nem o número de dias, as métricas agregadas permanecem o critério principal.

\section{Análise da Importância das Variáveis}

O gráfico de importância por ganho médio do XGBoost (Figura~\ref{fig:importancia}) mostra as variáveis que mais contribuíram para as predições do modelo. O número de interrupções do dia de origem (\texttt{interrupcoes}[$t$]) ocupa a primeira posição, refletindo que o estado imediato da rede é a informação mais preditiva para $h=1$. Na sequência aparecem \texttt{interrupcoes\_lag\_1}, \texttt{interrupcoes\_lag\_3}, \texttt{interrupcoes\_lag\_2}, \texttt{mes\_sin}, \texttt{mes\_cos} e a EMA de sete dias da precipitação.
...
\midrule
\textbf{Este trabalho} & Bi-GRU / Bi-LSTM & Regressão & MAE = 60,36; $R^2 = 0{,}421$ & Diário, Brasil (DF) \\
...
A elevação da média do alvo total de 183,5 interrupções/dia em 2017 para 303,4 no período disponível de 2025 tem implicações para a modelagem, mas não pode ser interpretada isoladamente como deterioração da rede. Os dados não separam mudanças de registro, exposição, infraestrutura e clima. Três hipóteses podem ser investigadas em trabalhos futuros:
...
Uma reflexão metodológica importante emerge da comparação entre o XGBoost e as redes recorrentes: \textbf{todos os modelos receberam o mesmo conjunto de variáveis disponíveis por instante e compartilharam o corte temporal}. A diferença é que o XGBoost processa uma linha na origem, enquanto Bi-LSTM e Bi-GRU processam janelas sequenciais de 14 dias. O resultado favorável ao XGBoost em $h=1$ não demonstra que a recorrência seja desnecessária em outros conjuntos, nem quantifica isoladamente o efeito da engenharia manual de atributos.
